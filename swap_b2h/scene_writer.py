...
## ToDo: Test this somehow (before trying it in the add-on)
def create_scenepart_swap(swap_list, id_string="", up_axis="z"):
    scenepart = f"""
        <part {id_string}>
    """
    # iterate over swap list
    for i, filepath in enumerate(swap_list):
        if i == 0:
            scenepart += f"""
            <filter type="objloader">
                <param type="string" key="filepath" value="{filepath}" />
        ...
        # for each epoch, swap the OBJ for the one from the new path
        else:
            scenepart += f"""
            <swap swapStep="1">
                <filter type="objloader">
